  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6">
      ...
      <ul className="space-y-4">
        {topics.map(topic => (
          <li key={topic._id} className="border rounded p-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              <div className="font-semibold truncate">{topic.title}</div>
              <div className="text-gray-600 text-sm mb-2 break-words">{topic.description}</div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {topic.status === 'accepted' && (
                <>
                  <button onClick={() => navigate(`/messages?topic=${topic._id}`)} className="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700">Chat</button>
